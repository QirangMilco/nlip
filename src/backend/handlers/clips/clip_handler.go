...
func HandleUploadClip(c *fiber.Ctx) error {
    ...
    // 如果有文件，保存到存储
    if req.File != nil {
        fileName := fmt.Sprintf("%s%s", cl.ID, filepath.Ext(req.FileName))
    ...
    _, err = config.DB.Exec(`
        INSERT INTO nlip_clipboard_items (id, space_id, content_type, content, file_path) 
        VALUES (?, ?, ?, ?, ?)
    `, cl.ID, cl.SpaceID, cl.ContentType, cl.Content, cl.FilePath)
...
func HandleListClips(c *fiber.Ctx) error {
    ...
    rows, err := config.DB.Query(`
        SELECT id, space_id, content_type, content, file_path, created_at 
    ...
    for rows.Next() {
        var cl clip.Clip
        err := rows.Scan(&cl.ID, &cl.SpaceID, &cl.ContentType, &cl.Content, &cl.FilePath, &cl.CreatedAt)
        if err != nil {
...
func HandleGetClip(c *fiber.Ctx) error {
    ...
    err := config.DB.QueryRow(`
        SELECT c.id, c.space_id, c.content_type, c.content, c.file_path, c.created_at,
               s.type as space_type, s.owner_id as space_owner_id
        FROM nlip_clipboard_items c
        JOIN nlip_spaces s ON c.space_id = s.id
        WHERE c.id = ?
    `, clipID).Scan(
        &cl.ID, &cl.SpaceID, &cl.ContentType, &cl.Content, &cl.FilePath, &cl.CreatedAt,
...
func HandleDeleteClip(c *fiber.Ctx) error {
    ...
    err := config.DB.QueryRow(`
        ...
        JOIN nlip_spaces s ON c.space_id = s.id
        WHERE c.id = ?
    `, clipID).Scan(&filePath, &spaceOwnerID)
    ...
    // 删除数据库记录
    _, err = tx.Exec("DELETE FROM nlip_clipboard_items WHERE id = ?", clipID)
    if err != nil {
        tx.Rollback()
